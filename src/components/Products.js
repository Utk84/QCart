...
const Products = () => {
  ...
  const debounceSearch = (event, debounceTimeout) => {
   ...
   useEffect(()=>{
    const onLoadHandeler=async()=>{
      const productsData= await performAPICall()
      const cardData=await fetchCart(token)
      console.log( " this is cart data",cardData)
  ...
  const updateCartItems=(cardData,products)=>{
    const cardItem=generateCartItemsFrom(cardData,products)
    console.log(" this is cardItem",cardItem)
    setItems(cardItem)
  }
  ...
  const addToCart = async (
   ...
   if(!token){
    enqueueSnackbar(
      "Please login first to add the item in card",
      {
        variant: "warning",
      }
    );
   ...
   try {
    ...
    if (e.response ) {
      ...
      enqueueSnackbar(
        "Could not Add cart details. Check that the backend is running, reachable and returns valid JSON.",
        {
          variant: "error",
        }
      );
